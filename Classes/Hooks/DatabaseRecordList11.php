 ...
 */
class DatabaseRecordList11 implements RecordListHookInterface, RecordListGetTableHookInterface, SingletonInterface
{
    /**
     * @var Iconfactory
     */
    ...
    public function checkChildren(string $table, array $row, int $level, array &$theData, DatabaseRecordListXclass $parentObj)
    {
        if ($table === 'tt_content' && $row['CType'] === 'gridelements_pi1') {
            $elementChildren = GridelementsHelper::getInstance()->getChildren(
                $table,
                $row['uid'],
                $row['pid'],
                '',
